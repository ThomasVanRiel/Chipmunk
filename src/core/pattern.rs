...
use crate::operations::OperationCommon;
...
impl Pattern {
    ...
    pub fn into_points(&self) -> Result<Vec<[f64; 3]>> {
        match self {
            Pattern::Circular {
                cc,
                diameter,
    ...
    pub fn into_segment(&self, common: &OperationCommon) -> Result<ToolpathSegment> {
        match self {
            Pattern::Circular {
                cc,
                diameter,
                ...
                let a_start = angle_start.unwrap_or(0.0_f64);
                let a_stop = angle_stop.unwrap_or(360.0_f64);

                let (resolved_count, resolved_step) = match (count, angle_step) {
                    (Some(_), Some(_)) => {
                        return Err(anyhow!(
                            "Circular pattern: specify count or angle_step, not both"
                        ));
                    }
                    (Some(n), None) => (n.unsigned_abs(), 0.0),
                    (None, Some(step)) => (0, *step),
                    (None, None) => {
                        return Err(anyhow!(
                            "Circular pattern: must specify count or angle_step"
                        ));
                ...
                Ok(ToolpathSegment {
                    move_type: MoveType::Pattern,
                    x: cc[0],
                    y: cc[1],
                    z: common.clearance,
                    comment: None,
                    pattern: Some(NCBlock::PatternCircular {
                        x: cc[0],
                        y: cc[1],
                        diameter: r * 2.0,
                        angle_start: a_start,
                        angle_stop: a_stop,
                        angle_step: resolved_step,
                        count: resolved_count,
                        clearance: *clearance,
                        surface_position: *surface_position,
                        second_clearance: common.clearance - surface_position,
                    }),
                })
            }
        }
